...
const FormInput =  forwardRef(({ label, containerStyle, error, ...inputProps }, ref) => {
  // 함수의 인자에 ...이 들어가면, 나머지 모든 매개변수(props)를 뜻함
  // 나머지 매개 변수를 쓴 이유는, 수 많은 input의 속성을 모두 명시할 수 없으므로 축약

  // 6
  // component화 시킨 요소에 ref를 부여할 때 forwadRef
  ...
  return (
    ...
    {/*{error & <p>{error}</p> */}
    <p style={{color: "red", fontSize: 10, visibility: error ? 'vislbie': 'hidden'}}>{error}</p>
    </>
  );
});
